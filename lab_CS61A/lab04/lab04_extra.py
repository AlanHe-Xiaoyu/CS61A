...
def flatten(lst):
    ...
    return [item for i in lst for item in flatten(i)] if type(lst) == list else [lst]
...
def create_board(rows, columns):
    ...
    for i in range(rows):
        answer.append(create_row(columns))
...
def put_piece(board, max_rows, column, player):
    ...
    for row_num in range(max_rows - 1, -1, -1):
        cur_piece = get_piece(board, row_num, column)
        if cur_piece == '-':
...
def print_board(board, max_rows, max_cols):
    ...
    for row in range(max_rows):
        ...
        for col in range(max_cols):
            cur_col += get_piece(board, row, col) + ' '

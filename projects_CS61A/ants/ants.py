    ...
    def add_insect(self, insect):
        ...
        if insect.is_ant:
            if self.ant is None:
                ...
                # BEGIN Problem 9
                cur_ant = self.ant
                if cur_ant.is_container and cur_ant.can_contain(insect):
                    cur_ant.contain_ant(insect)
                elif insect.is_container and insect.can_contain(cur_ant):
                    insect.contain_ant(cur_ant)
                    self.ant = insect # Set container to the Ant in this Place
                else:
...
class Bee(Insect):
    ...
    is_watersafe = True
    reverse_direction = False # False as forward, True as backward
    has_not_reversed = True
    ...
    def blocked(self):
        ...
        return ant_here is not None and ant_here.blocks_path
        # END Problem 7
    ...
    def action(self, colony):
        ...
        # BEGIN EC
        "*** YOUR CODE HERE ***"
        if self.reverse_direction: # If direction reversed
            destination = self.place.entrance
            if destination is not colony.hive:
...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self, hive, min_range=0, max_range=float('inf')):
        ...
        view_place = self.place

        # Set the first checking place to the Place at min_range
        while cur_view < min_range:
            if view_place.entrance != hive:
                view_place = view_place.entrance
                cur_view += 1
            else: # if its min_range points to hive
                return None
        
        # Move pointer cur_view by 1 and check current Place
        cur_view += 1
        if view_place.bees: 
            return random_or_none(view_place.bees)

        # Loop through
        while view_place.entrance != hive and cur_view <= max_range:
            view_place = view_place.entrance
            if view_place.bees:
                return random_or_none(view_place.bees)
            cur_view += 1
...
class HungryAnt(Ant):
    ...
    def action(self, colony):
        ...
        if self.digesting:
            ...
            pick_bee = random_or_none(self.place.bees)
            self.eat_bee(pick_bee)
        # END Problem 6
...
# The WallAnt class
class WallAnt(Ant):
    """WallAnts! Heavily armored."""

    name = 'Wall'
    food_cost = 4
    implemented = True

    # Create a WallAnt with default armor=4
...
class BodyguardAnt(Ant):
    ...
    def can_contain(self, other):
        # BEGIN Problem 9
        "*** YOUR CODE HERE ***"
        empty = self.contained_ant == None
        not_container = not other.is_container
        return empty and not_container
        # END Problem 9
...
class TankAnt(BodyguardAnt):
    ...
    def action(self, colony):
        # BEGIN Problem 10
        "*** YOUR CODE HERE ***"
        # First, kust like Bodyguard, let the contained Ant does its job
        BodyguardAnt.action(self, colony)

        # Then deals self.damage to all bees in its place
        all_bees = self.place.bees[:]
        for bee in all_bees:
            bee.reduce_armor(self.damage)
...
class ScubaThrower(ThrowerAnt):
    ...
    # Create a ScubaThrower as a ThrowerAnt with default armor=1
    def __init__(self, armor=1):
...
# END Problem 13
    ...
    def action(self, colony):
        ...
        # BEGIN Problem 13
        "*** YOUR CODE HERE ***"
        # Helper function to buff an Ant if necessary
        def buff_ant(cur_ant):
            if not cur_ant.is_buffed:
                cur_ant.damage *= 2
                cur_ant.is_buffed = True

        if not self.true_queen:
            self.reduce_armor(self.armor)
        else:
            ScubaThrower.action(self, colony) # Throws a leaf
            view_place = self.place
            while view_place.exit:
                view_place = view_place.exit # Move back one Place
                ant_exist = view_place.ant # Check if Ant exists in the Place
                if ant_exist:
                    # If contains any other Ant, then buff the inside Ant
                    if ant_exist.is_container and ant_exist.contained_ant:
                        buff_ant(ant_exist.contained_ant)
                    # Buffs the outside Ant
                    buff_ant(ant_exist)
        # END Problem 13
...
def make_slow(action, bee):
    ...
    # BEGIN Problem EC
    "*** YOUR CODE HERE ***"
    def slow_helper(colony):
        to_pause = colony.time % 2 == 1
        if not to_pause:
...
def make_scare(action, bee):
    ...
    def scare_helper(colony):
        if bee.has_not_reversed:

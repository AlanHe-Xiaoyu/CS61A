...
def play(strategy0, strategy1, score0=0, score1=0, dice=six_sided,
    ...
    while score0 < goal and score1 < goal:
        if player == 0:
            # take_turn takes three parameters: num_rolls, opponent_score, dice
            strat_roll = strategy0(score0, score1)
            score0 += take_turn(strat_roll, score1, dice)
        else:
            strat_roll = strategy1(score1, score0)
            score1 += take_turn(strat_roll, score0, dice)

        if is_swap(score0, score1): # Check for a Swine Swap
...
def announce_highest(who, previous_high=0, previous_score=0):
    ...
    def say(score0, score1):
        ...
        current_high = current_score - previous_score # Setting the current point different

        if current_high > previous_high: # Check for new high
            print(current_high, "point(s)! That's the biggest gain yet for Player", who)

        current_high = max(previous_high, current_high) # Setting the new highest gain
...
def make_averaged(fn, num_samples=1000):
    ...
    def dice_averaged(*args):
        total = 0 # Initializing sum
        for i in range(num_samples):
            total += fn(*args)
        return total / num_samples

    return dice_averaged
...
def max_scoring_num_rolls(dice=six_sided, num_samples=1000):
    ...
    for cur_rolls in range (1,11):
        cur_average = make_averaged(roll_dice, num_samples)(cur_rolls, dice)
        
        if cur_average > max_score:
            max_score, max_rolls = cur_average, cur_rolls
...
def swap_strategy(score, opponent_score, margin=8, num_rolls=4):
    ...
    if is_swap(score, opponent_score): # if Swine Swap applies
        swap_gain = opponent_score - (score - free_bacon_score) # Difference between opponent_score and previous_score
        if swap_gain >= 0:
            return 0
        else:
            return num_rolls
    else:
        return bacon_strategy(score - free_bacon_score, opponent_score, margin, num_rolls)
...
def final_strategy(score, opponent_score):
    ...
    *** YOUR DESCRIPTION HERE ***
    ...
    if is_swap(temp_score, opponent_score): # Trying free_bacon strategy
